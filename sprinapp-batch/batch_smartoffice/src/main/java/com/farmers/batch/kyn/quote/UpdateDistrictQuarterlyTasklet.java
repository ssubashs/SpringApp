...
@Service("updateDistrictQuarterly")
public class UpdateDistrictQuarterlyTasklet implements Tasklet{
	...
	public RepeatStatus execute(StepContribution stepArgs, ChunkContext chunkArgs)
		...
		try
		{
			System.out.println("update district quarterly :: Starts ");
	...
	private void updateforQuarter(Integer quarter) {
		...
		String queryStr = "select id.state,id.district,id.lob,id.year,sum(quotes) as quotes, sum(pif) as pif, sum(newbusiness) as newbusiness, avg(retention) as retention, avg(closerate) as closerate FROM Districtmonthly where id.month <=:currentquarter *3 and month >=(:currentquarter-1)*3 group by id.state,id.district,id.lob,id.year";
		...
		{			
			...
			for(int i=0;i<row.length;i++)
			{
				...
				case 1:
					if(row[i]!=null)
					{
					dummyview.setDistrict((String)row[i]);
					}
					break;
				case 2:
					if(row[i]!=null)
	...
	public void updatelikeviews(Reportview dummyview) {
	  ...
	  {  
        tx.begin();
		String queryStr = "update Reportview SET districtdata = :data where state = :state and district = :district and id.datacategory = :datatype and id.lob = :lob and id.quarter = :quarter";
		Query query = em.createQuery(queryStr);
		query.setParameter("data", dummyview.getDistrictdata());
		query.setParameter("state", dummyview.getState());
		query.setParameter("district", dummyview.getDistrict());
      ...
      System.out.println("Error trying to delete all Rechtsvorgang" + ex.getMessage());
       try {
          tx.rollback();
