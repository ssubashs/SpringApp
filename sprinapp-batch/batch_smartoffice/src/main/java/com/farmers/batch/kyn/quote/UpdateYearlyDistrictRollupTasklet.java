...
@Service("yearlyDistrictRollups")
public class UpdateYearlyDistrictRollupTasklet implements Tasklet{
...
//				this.updateAgentforYear(year);
	...
	private void updateAgentforYear() {
		...
		String queryStr = "select id.state,id.district,id.lob,id.year,sum(quotes) as quotes, sum(pif) as pif, sum(newbusiness) as newbusiness, avg(retention) as retention, avg(closerate) as closerate FROM Districtmonthly group by id.state,id.district,id.lob,id.year";
...
//			dummyview.getId().setYear(year);
	...
	public void updatelikeviewsagentdata(Reportview dummyview) {
	  ...
	  {  
        tx.begin();
		String queryStr = "update Reportview SET pastdistrictdata = :data where state = :state and district = :district and id.datacategory = :datatype and id.lob = :lob and id.year = :year";
