...
@Service("yearlyAgentRollups")
public class UpdateYearlyAgentRollupTasklet implements Tasklet{
...
//				this.updateAgentforYear(year);
	...
	private void updateAgentforYear() {
		...
		String queryStr = "SELECT monthly.state,monthly.district,monthly.id.lob,monthly.id.year,sum(monthly.quotes) as quotes, sum(monthly.pif) as pif, sum(monthly.newbusiness) as newbusiness,avg(retention) as retention, avg(closerate) as closerate FROM Agentmonthly monthly  group by monthly.state,monthly.district,monthly.id.lob,monthly.id.year";
...
//			dummyview.getId().setYear(year);
	...
	public void updatelikeviewsagentdata(Reportview dummyview) {
	  ...
	  {  
        tx.begin();
		String queryStr = "update Reportview SET pastagentdata = :data where state = :state and district = :district and id.datacategory = :datatype and id.lob = :lob and id.year = :year";
